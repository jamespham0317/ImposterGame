...
import { buttonVariants, listContainerVariants, listItemVariants, modalContentVariants, modalVariants } from "../utils/animations.ts";
...
export default function BriefingPanel() {
    ...
    return (
        <>
            <motion.div className="fixed inset-0 z-40 flex items-center justify-center" variants={modalVariants} initial="hidden" animate="visible" exit="exit">
                <motion.div className="absolute inset-0 bg-black/60 backdrop-blur-sm" variants={modalVariants} />

                {isBriefingOpen && (
                    <motion.div className="relative z-50 mx-4 w-full max-w-4xl rounded-3xl border border-gray-700 bg-brand-gray shadow-xl overflow-hidden" variants={modalContentVariants} initial="hidden" animate="visible" exit="exit">
                        <motion.div className="border-b border-gray-700 px-6 py-4" variants={listItemVariants}>
                        ...
                        <motion.div className="p-6" variants={listContainerVariants} initial="hidden" animate="visible">
                            <div className="min-w-0">
                                {username === imposter ? (
                                    <motion.div className="space-y-4" variants={listContainerVariants}>
                                        <motion.div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-5 py-6 text-center" variants={listItemVariants}>
                                        ...
                                        <motion.div className="rounded-xl border border-gray-700 bg-brand-gray-light/40 p-4" variants={listItemVariants}>
                                    ...
                                    <motion.div className="space-y-4" variants={listContainerVariants}>
                                        ...
                                        <motion.div className="max-h-[60vh] space-y-4 overflow-y-auto pr-1 text-gray-300 custom-scrollbar" variants={listContainerVariants}>
                        ...
                        <motion.div className="flex justify-end border-t border-gray-700 px-6 py-4 bg-brand-gray" variants={listItemVariants}>
                            <motion.button
                                type="button"
                ...
                {isReady && (
                    <motion.div className="relative z-50 mx-4 w-full max-w-md rounded-3xl border border-gray-700 bg-brand-gray px-6 py-7 text-center shadow-xl" variants={modalContentVariants} initial="hidden" animate="visible" exit="exit">
                        <p className="text-xs font-semibold uppercase tracking-widest text-gray-500">Stand By</p>
                        <motion.div className="mt-2 inline-block rounded-full border border-gray-700 bg-brand-gray-light/40 px-2 py-1 text-xs font-semibold uppercase tracking-widest text-gray-400" animate={{ scale: [1, 1.04, 1] }} transition={{ duration: 1.2, repeat: Infinity }}>
                            {briefingTime}s
                        </motion.div>
